...
sys_exofork(void)
{
	...
	struct Env *new_env;
	int flag;
	if((flag = env_alloc(&new_env, sys_getenvid())) < 0)
		return flag;

	new_env->env_tf = curenv->env_tf;//copy the trapframe
	new_env->env_status = ENV_NOT_RUNNABLE;
	//tweaked so sys_Exofork will apear to return 0
	new_env->env_tf.tf_regs.reg_eax = 0;//use eax as the return value
...
sys_page_alloc(envid_t envid, void *va, int perm)
{
	...
	// LAB 4: Your code here.
	struct Env *e;
	//check the env correct, only parent has permision
	if(envid2env(envid, &e, 1) < 0)
		return -E_BAD_ENV;
	//check va
	...
	if(!pg)
		return -E_NO_MEM;
	int ret = page_insert(e->env_pgdir, pg, va, perm);
	// If page_insert() fails, remember to free the page you
	//   allocated!
	if(ret)//lack of memory
	{
		page_free(pg);
		return ret;
	}
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	// check srcva space
	if(!(pp = page_lookup(src->env_pgdir, srcva, &pte)))
		return -E_INVAL;
...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
	...
	if(envid2env(envid, &dstenv, 0) < 0)
		return -E_BAD_ENV;
	// check envid blocked in sys_ipc_recv?
	if(!dstenv->env_ipc_recving)
		return -E_IPC_NOT_RECV;
	if((uintptr_t)srcva < UTOP)//if srcva < UTOP
	{
...
{
	// LAB 4: Your code here.
	//if dstva < UTOP then willing to receive a page of data
